  ...
  async viewprofile(@Session() session): Promise<ProfileEntity>  {
    ...
    if (user)
    {
      if (user.role === 'Admin')
      {
        ...
        // const res = await this.profileservice.viewprofile(29);
        return res;
      }
    }
...
@UseGuards(SessionGuard)
async removeDistributorfromRedList(@Query('serialnumber')serialnumber:number, @Session() session): Promise<{ message: string } | { success: boolean }>{ 
  const user = session.user;
  if (user) {
    if (user.role === 'Admin') {
      try {
...
@UseGuards(SessionGuard)
async removeIndustryfromRedList(@Query('serialnumber')serialnumber:number, @Session() session): Promise<{ message: string } | { success: boolean }>{ 
  const user = session.user;
  if (user) {
    if (user.role === 'Admin') {
      try {
